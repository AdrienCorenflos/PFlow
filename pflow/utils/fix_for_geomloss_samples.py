...
class Softmin(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, epsilon, C, f):
        B = C.shape[0]
        temp = (f.view(B, 1, -1) - C / epsilon)
        ctx.save_for_backward(torch.tensor(epsilon), temp)
        return - epsilon * temp.logsumexp(2).view(-1)

    @staticmethod
    def backward(ctx, grad_output):
        epsilon, temp = ctx.saved_tensors
        B = temp.shape[0]
        max_ = temp.max(2).values
        temp = temp - max_.unsqueeze(2)
        temp_exp = temp.exp()
        temp_exp_sum = temp_exp.sum(2)
...
def sinkhorn_tensorized(α, x, β, y, p=2, blur=.05, reach=None, diameter=None, scaling=.5, cost=None,
    ...
    if is_log:
        ...
        log_alpha, log_beta = log_weights(α), log_weights(β)
        alpha, beta = α, β
