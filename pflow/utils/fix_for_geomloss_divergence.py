...
def sinkhorn_cost(ε, ρ, α, β, a_x, b_y, a_y, b_x, batch=False, debias=True, potentials=False, is_log=False):
    ...
    # Classic, BIASED entropized Optimal Transport OT_ε(α,β)
    if ρ is None:
...
def sinkhorn_loop(softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ,
    ...
    if last_extrapolation:
        ...
        if debias:
            a_x = λ * softmin(ε, C_xx, α_log + a_x.detach() / ε)
            b_y = λ * softmin(ε, C_yy, β_log + b_y.detach() / ε)

        # The cross-updates should be done in parallel!
        a_y, b_x = λ * softmin(ε, C_yx, α_log + b_x.detach() / ε), \
                   λ * softmin(ε, C_xy, β_log + a_y.detach() / ε)
